//----------------------------------------------------------------
//
// THIS TEST IS USED WITH OSCILLOSCOPE ON PIN 5 OF PCB
// PIN 5 IS THE CORNER ONE NEXT TO ICSP CONNECTOR
// THE PIN WILL GO HIGH AND STAY HIGH IF INITIALIZATION WORKED
//
//
//----------------------------------------------------------------
/*   
...
void setup() {
   ...
   //   pinMode      (nrfPower           , OUTPUT) ;      digitalWrite  (nrfPower          , LOW   ) ;
      pinMode      (firstInterruptPin  , INPUT ) ;        digitalWrite  (firstInterruptPin , LOW   ) ;     // Set up the interrupt pins, active high, push-pull 
      pinMode      (secondInterruptPin , INPUT ) ;  digitalWrite  (secondInterruptPin, LOW    )  ;
   
      // pin indicating success of test
      pinMode(led, OUTPUT);     
...
void sendDataToServer (unsigned int data)  {
           ...
           //Serial.println(data);
            ...
            Mirf.cePin  =  8 ;
            Mirf.csnPin = 10 ;  //53; mega
            Mirf.init();
          unsigned long tutu;
            Mirf.setRADDR((byte *)SENSOR_ADDRESS);
            Mirf.payload = sizeof(DATA_PAYLOAD) ; 
            Mirf.config();
