...
void i2cInit(void) {
	i2cSetBitrate(I2C_SPEED);   // set i2c bit rate to 40KHz
...
void i2cSetBitrate(unsigned short bitrateKHz)
{
	...
	//calculate bitrate division	
        // F_CPU => 16000000 on Arduino mega
...
void i2cSendStart(void)
{
	WRITE_sda(); 	TWCR = (1<<TWINT)|(1<<TWSTA)|(1<<TWEN); 	// send start condition
}
...
void i2cWaitForComplete(void)
{
	...
	int timeToStopWaiting = 90   ;
	// wait for i2c interface to complete operation
    while ((!(TWCR & (1<<TWINT))) && (i < timeToStopWaiting))
		i++;
}
...
unsigned char i2cGetReceivedByte(void){	return( inb(TWDR) );    } // retieve received data byte from i2c TWDR 

unsigned char i2cGetStatus(void){         	return( inb(TWSR) );     } // retieve current i2c status from i2c TWSR
